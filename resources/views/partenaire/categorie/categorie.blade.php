...
<main class="bg-gradient-to-r from-red-100 via-red-300 to-red-100 pb-28  relative top-[120px] sm:top-[133px] md:top-[85px] lg:top-[82px] md:left-[160px] md:max-w-[77vw] lg:max-w-[82vw] ">
    ...
    <div class="grid grid-cols-1 md:grid-cols-3">
            ...
            <div class="grid grid-cols-1 place-items-center gap-5 sm:grid-cols-2 lg:grid-cols-3">
                     ...
                     @forelse($categories as $categorie)
                        ...
                        <div class="bg-green-300/50 hover:bg-green-300/90 hover:duration-700 max-w-[180px] flex flex-col justify-center items-center space-y-0 rounded-2xl relative">
                            <span class="absolute top-0 inset-x-0 text-center text-sm font-thin z-10">{{$categorie.created_at}} </span>
                            <img class="w-[180px] h-[180px] cursor-grab object-cover rounded-full duration-700 scale-[65%] hover:scale-75" src="">
                            <h3 class="text-lg font-bold pb-5">{{$categorie.libele}}</h3>
                            <a href=""><i class="absolute top-1 left-2 text-yellow-500 font-black text-lg h-8 w-8 bg-sky-200 rounded-full cursor-pointer flex items-center justify-center hover:bg-sky-600 hover:duration-700 fa fda-spin fa-cog"></i></a>
                            <a onclick="return confirm('Est-vous sur de continuer ?')" href=""><i class="absolute top-1 right-2 text-red-400 font-black text-lg h-8 w-8 p-1 bg-red-100 rounded-full flex items-center justify-center cursor-pointer hover:bg-red-600 hover:text-white hover:duration-700 fa fa-trash-o"></i></a>
                        </div>

                        @empty

                        <h1 class="flex mt-4 items-center justify-center py-4 bg-blue-500 text-bold text-white">Aucun client n'es ajouter</h1>
        ...
        <form class="bg-red-100 p-2 order-first md:order-last md:fixed md:right-0 md:top-20 md:w-[26%] lg:w-[28%] xl:w-[33%] z-40" enctype="multipart/form-data" method="post">
    <fieldset class="group p-2 border-2 min-h-[200px] border-black rounded-lg grid grid-cols-3 gap-2 md:grid-cols-1">
        ...
        <div>
            ...
            <input type="text" name="libelle" id="libelle" class="mt-1 p-2 block w-full rounded-md shadow-sm border-gray-300 focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50" value="{{ old('libelle') }}" required autofocus />
        </div>
